...
from meteostat import Base, Point, Daily, units
...
Base.cache_dir = os.path.join(os.path.dirname(__file__), ".meteostat", "cache")
...
# Photo icon
images = glob.glob("GPX-Reporter/photos/*.jpg")
for i in images:
    e = exif(i)
...
# Build the elevation chart locally.
...
left, top, right, bottom = 65, 20, 20, 45
plot_w = chart_w - left - right
plot_h = chart_h - top - bottom
min_elv = min(elvs)
max_elv = max(elvs)
elv_range = max(max_elv - min_elv, 1)
chart_img = Image.new("RGB", (chart_w, chart_h), "white")
chart_draw = ImageDraw.Draw(chart_img)
font = ImageFont.load_default()
chart_draw.rectangle(
    [left, top, chart_w - right, chart_h - bottom],
    outline="black",
    fill=(246, 248, 242),
)
plot_points = []
for i, elv in enumerate(elvs):
    x = left + ((i / max(len(elvs) - 1, 1)) * plot_w)
    y = top + ((max_elv - elv) / elv_range * plot_h)
    plot_points.append((int(x), int(y)))
fill_points = [(left, chart_h - bottom)] + plot_points + [(chart_w - right, chart_h - bottom)]
chart_draw.polygon(fill_points, fill=(128, 198, 90))
chart_draw.line(plot_points, fill="black", width=2)

mid_elv = min_elv + (elv_range / 2)
for value in (min_elv, mid_elv, max_elv):
    y = top + ((max_elv - value) / elv_range * plot_h)
    chart_draw.line([(left - 5, y), (left, y)], fill="black")
    chart_draw.text((5, y - 6), str(int(round(value))), fill="black", font=font)
chart_draw.text((5, top + (plot_h / 2) - 6), "FEET", fill="black", font=font)

for mile, pos in zip(miles, positions):
    x = left + ((pos / 100.0) * plot_w)
    chart_draw.line([(x, chart_h - bottom), (x, chart_h - bottom + 5)], fill="black")
    chart_draw.text((x - 4, chart_h - bottom + 8), str(mile), fill="black", font=font)
chart_draw.text((left + (plot_w / 2) - 15, chart_h - 20), "MILES", fill="black", font=font)
chart_img.save('{}_profile.png'.format(elv_img))
